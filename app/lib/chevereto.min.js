...
CHV.fn.bindSelectableItems();});if($("#home-cover, #maintenance-wrapper").exists()){if($("#maintenance-wrapper").exists()){var landing_src=$("#maintenance-wrapper").css("background-image").slice(4,-1).replace(/^\"|\"$/g,"");}else{var landing_src=$(".home-cover-img","#home-cover-slideshow").first().attr("data-src");}
function showHomeCover(){$("body").addClass("load");if(!$("#maintenance-wrapper").exists()){$(".home-cover-img","#home-cover-slideshow").first().css("background-image","url("+landing_src+")").addClass("animate-in--alt").removeAttr("data-src");}
...
var data=XHR.responseJSON.software;if(PF.fn.versionCompare(CHV.obj.system_info.version,data.current_version)==-1){PF.fn.modal.simple({title:PF.fn._s("Update available v%s",data.current_version),message:'<p>'+PF.fn._s('There is an update available for your system. You can automatic download and install this update or go to %s to proceed to download the file.','<a href="http://chevereto.com/panel/downloads" target="_blank">chevereto.com</a>')+'<p>'+PF.fn._s('The release notes for this update are:')+'</p>'+'<textarea class="r4 resize-vertical">'+data.release_notes+'</textarea>'+'<div class="btn-container margin-bottom-0"><a href="'+PF.obj.config.base_url+'/update'+'" class="btn btn-input default">'+PF.fn._s('Update now')+'</a> <span class="btn-alt">'+PF.fn._s('or')+' <a data-action="cancel">'+PF.fn._s('cancel')+'</a></span></div>',html:true});}else{PF.fn.growl.call(PF.fn._s("Your website is running the latest version of Chevereto."));}});});if(typeof PF.fn.get_url_var("checkUpdates")!==typeof undefined){$("[data-action=check-for-updates]").click();}
...
$("#top-bar-shade").animate({opacity:0},animation.time,animation.easing,function(){if(!is_slim_shady){$(this).remove();}});}
if(!$("#top-bar").hasClass("transparent")){fade_slim_shady()}
