...
<div class="c7 input-label">
    ...
    <input type="text" name="<?php echo $label; ?>" class="text-input" value="<?php echo $album["name"]; ?>" placeholder="<?php _se('Album name'); ?>" maxlength="<?php echo CHV\getSetting('album_name_max_length'); ?>" required>
	<?php if (CHV\Login::isLoggedUser() && $GLOBALS['theme_include_args']['album-switch'] !== false) {
        ?>
	...
	<div data-combo-value="password" class="switch-combo<?php echo($album['privacy'] !== 'password' ? ' soft-hidden' : null); ?>">
		<div class="input-label overflow-auto">
			<div class="c7 grid-columns">
				...
				<input type="text" name="form-album-password" class="text-input" value="<?php echo $album['password']; ?>" data-required<?php echo($album['privacy'] == 'password' ? ' required' : null); ?>>
			</div>
		</div>
	</div>
</div>
